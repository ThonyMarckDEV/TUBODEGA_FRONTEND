// src/services/cajeroService.js
import { fetchWithAuth } from 'js/authToken'; 
import API_BASE_URL from 'js/urlHelper';
import { handleResponse } from 'utilities/Responses/handleResponse'; 

export const createCajero = async (data) => {
  const url = `${API_BASE_URL}/api/cajeros/store`;
...
export const showCajero = async (id) => {
  const url = `${API_BASE_URL}/api/cajeros/show/${id}`;
  const response = await fetchWithAuth(url, { method: 'GET' });
  return handleResponse(response);
};

export const updateCajero = async (id, data) => {
  const url = `${API_BASE_URL}/api/cajeros/update/${id}`;
  const response = await fetchWithAuth(url, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
    body: JSON.stringify(data)
...
export const toggleCajeroEstado = async (id, nuevoEstado) => {
    const url = `${API_BASE_URL}/api/cajeros/cambiar-estado/${id}`;
    const response = await fetchWithAuth(url, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
